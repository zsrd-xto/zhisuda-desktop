...
- [x] 岗位列表 UI：勾选、黑名单、重点关注
- [x] L1 批量投递（`delivery:applyBatch`，≤10/次，15–30s 间隔）
- [x] 投递状态机：待投递 → 投递中 → 已投递 / 失败
- [x] 人工接管模式（CAPTCHA / RATE_LIMIT / DOM_CHANGED）
- [x] 投递记录持久化（`delivery:getRecords`）
- [x] 基础数据看板：总投递、今日投递、简单趋势
